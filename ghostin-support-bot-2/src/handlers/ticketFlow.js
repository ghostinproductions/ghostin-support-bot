...
async function createTicketChannel(guild, user, lang, answers) {
  ...
  const ticketNumber = storage.getNextTicketNumber(guild.id);
  const ticketLabel = String(ticketNumber).padStart(4, '0');
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: t(lang, 'fieldTicketNumber'), value: `#${ticketLabel}`, inline: true },
